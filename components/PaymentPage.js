  ...
  return (
    <>
      ...
      <div className='cover w-full bg-red-50 relative'>
        <img className='object-cover w-full h-48 md:h-140' src={currentUser.coverpic} alt="" />
        <div className='absolute -bottom-20 right-[34%] md:right-[47.5%] border-white border-2 rounded-full'>
          <img className='rounded-full' width={150} height={150} src={currentUser.profilepic} alt="" />
        </div>
      </div>
      <div className='info flex justify-center items-center my-24 mb-24 flex-col gap-2'>
        ...
        <div className='payment flex gap-3 w-[80%] mt-11 flex-col md:flex-row'>
          <div className='supporters w-full md:w-1/2 rounded-lg bg-slate-900 text-white p-10'>
            ...
            <ul className='mx-5 text-lg'>
              ...
              {payments.map((p, i) => {
                return <li key={i} className='my-4 flex gap-2 items-center'>
                  ...
                  <span>
                    {p.name} donated <span className='font-bold'>₹{p.amount}</span> with a message &quote;{p.message}&quote;
                  </span>
                </li>
              })}
            </ul>
